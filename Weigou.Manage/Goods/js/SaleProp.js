                ...
                if (a.GoodsSaleProp[i].ID == data_id) {
                    switch (strTxtName) {
                        ...
                        case "Txt_MarketPrice":
                            ...
                            break;;
                        case "Txt_Stock":
                            a.GoodsSaleProp[i].Stock = strValue;
                            break;;
                    }
                }
            }
        ...
        for (var i = 0; i < a.GoodsSaleProp.length; i++) {
            ...
            $("input[name='hideSaleProp']").each(function (index, elemen) {
                if ($(this).val() == a.GoodsSaleProp[i].SaleProp) {
                    ...
                    $(this).parent().next().next().find("input[name='Txt_Stock']").val(a.GoodsSaleProp[i].Stock);//市场价格
                    $(this).parent().next().next().find("input[name='Txt_Stock']").attr("data_id", a.GoodsSaleProp[i].ID);
            ...
            if (bCheck == true) {
                ...
                if (zuheDate.length > 0) {
                    ...
                    $.each(zuheDate, function (index, item) {
                        ...
                        td2.appendTo(tr);
                        var td3 = $("<td ><input name=\"Txt_Stock\" data-options=\"required:true\" min=\"0\"  missingmessage=\"请输入商品库存数量\" class=\"easyui-numberbox  l-text txt\" type=\"text\" value=\"0\"></td>");
                        td3.appendTo(tr);
                    });
                }
                //结束创建Table表
