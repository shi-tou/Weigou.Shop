...
{
    public partial class SmsTemplateList : ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (!CheckAuth("SmsTemplateList"))
                    return;
                BindList();
            }
        }
        ...
        protected void repData_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                int res = 0;
                DataTable dt = sysService.GetDataByKey("T_SmsTemplate", "ID", id);
                if (dt.Rows.Count > 0)
                {
                    if (dt.Rows[0]["IsSystem"] == "1")
                    {
                        ShowMsg(this.Request.Url.ToString(), "系统模板不可以删除！");
                        return;
                    }
                    else
                    {
