...
namespace Weigou.Pay.Alipay.Pay
{
    /// <summary>
    /// 类名：Core
    /// 功能：支付宝接口公用函数类
    ...
    public class AlipayCore
    {
        ...
        public static string CreateLinkString(Dictionary<string, string> dicArray)
        {
            ...
            {
                //prestr.Append(string.Format("{0}=\"{1}\"&",temp.Key,temp.Value));
        ...
        /// <returns></returns>
        public static string CreateSign(SortedDictionary<string, string> dicArray)
        {

            //过滤空值、sign与sign_type参数
            Dictionary<string, string> dicSign = FilterPara(dicArray);
            //获取待签名字符串
            string strSign = AlipayCore.CreateLinkString(dicSign);
            //生成签名
            return RSAFromPkcs8.sign(strSign, AlipayConfig.Private_key, AlipayConfig.Input_charset);
        }
    }
}