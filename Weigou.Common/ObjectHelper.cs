...
namespace Weigou.Common
{
    public sealed class ObjectHelper
    {
        ...
        /// <summary>
        /// 循环添加Datatable列中的图片路径值
        /// </summary>
        /// <param name="dt"></param>
        /// <param name="t"></param>
        /// <param name="obj"></param>
        /// <returns></returns>
        public static DataTable UpdateDatatableField_Pic(DataTable dt, string strField)
        {
            if (dt == null)
                return dt;

            foreach (DataRow dr in dt.Rows)
                if (!string.IsNullOrEmpty(Convert.ToString(dr[strField])))
                    dr[strField] = Utils.GetConfig("ServerPicPath") + Convert.ToString(dr[strField]);
