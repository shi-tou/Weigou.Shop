...
(function($){
	...
	function buildGrid(target){
		...
		$(target).datagrid($.extend({}, opts, {
			cls:'propertygrid',
			view:(opts.showRole ? groupview : undefined),
...
//		}
	...
	$.fn.propertygrid.parseOptions = function(target){
		var t = $(target);
		return $.extend({}, $.fn.datagrid.parseOptions(target), $.parser.parseOptions(target,[{showRole:'boolean'}]));
	};
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
			...
			view.find('div.datagrid-group').each(function(){
				...
				$(this).find('span.datagrid-row-expander').bind('click', {groupIndex:groupIndex}, function(e){
					if ($(this).hasClass('datagrid-row-collapse')){
						$(target).datagrid('collapseRole', e.data.groupIndex);
					} else {
						$(target).datagrid('expandRole', e.data.groupIndex);
					}
				});
			});
		},
		...
		onBeforeRender: function(target, rows){
			...
			for(var i=0; i<rows.length; i++){
				var row = rows[i];
				var group = getRole(row[opts.groupField]);
				if (!group){
					group = {
	...
	$.extend($.fn.datagrid.methods, {
	    expandRole:function(jq, groupIndex){
	    ...
	    collapseRole:function(jq, groupIndex){
	        return jq.each(function(){
	            var view = $.data(this, 'datagrid').dc.view;
	            if (groupIndex!=undefined){
	                var group = view.find('div.datagrid-group[group-index="'+groupIndex+'"]');
