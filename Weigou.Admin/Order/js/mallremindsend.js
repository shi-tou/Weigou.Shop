...
function GetWhere() {
    var params = {//, 'MemberName': $('#txtMemberName').val(),'MobileNo': $('#txtMobileNo').val(), 'OrderStatus': $('#ddlOrderStatus').val()
        'action': 'GetMallRemindSendList', 'OrderNo': $('#txtOrderNo').val(), 'MinTime': $('#txtMinTime').val(), 'MaxTime': $('#txtMaxTime').val()
    };
    return params;
}
//获取商城订单列表
    ...
    tab.datagrid({
        ...
        columns: [[
           // { field: 'ID', title: '', width: 30, align: 'center', checkbox: true },
            ...
            { field: 'OrderStatus', title: '订单状态', width: 150, align: 'center', formatter: FormatOrderStatus },
...
function FormatOrderStatus(v) {
    ...
    switch (parseInt(v)) {
        ...
        case 10:
            ...
            break;
        case 20:
            res = '<span style="color:#6aa84f">已付款，待发货</span>';
            break;
        case 30:
